...
class PostService {
    constructor() {
        ...
        this.createPost = async (postData, userId) => {
        ...
        this.updatePost = async (postUpdateData, userId, postId) => {
            const post = await this.postModel.findOneAndUpdate({
                _id: postId,
                user: userId,
            }, { ...postUpdateData }, { new: true });
            if (!post)
                new Error("작성하신 글이 존재하지 않습니다.");
            return post;
        };
        this.deletePost = async (userId, postId) => {
            const post = await Post_1.PostModel.findOneAndDelete().and([
                { user: userId },
                { _id: postId },
            ]);
            if (!post)
                throw new Error("작성하신 글이 존재하지 않습니다.");
        };
        ...
        //포스트 가져오기(검색 포함!)
        this.getPosts = async (page2, meeting, category, keyword) => {
        ...
        this.addParticipant = async (postId, userId, participantId) => {
            ...
            if (!participant)
                throw new Error("잘못된 참가자 정보입니다.");
            if (!post)
                throw new Error("잘못된 정보가 기재되었습니다."); // postId 또는 작성자(user)가 잘못된 경우
            ...
            //참여인원에 추가하기
            await this.postModel.findByIdAndUpdate({ _id: post._id }, { $push: { participants: participant._id } });
        };
        this.deleteParticipant = async (postId, userId, participantId) => {
            const [participant, post] = await Promise.all([
                this.userModel.findOne({ _id: participantId }),
                this.postModel.findOne({ _id: postId, user: userId }),
            ]);
            ...
            //참여 인원에서 삭제하기
            await this.postModel.findByIdAndUpdate({ _id: post._id }, { $pull: { participants: participant._id } });
        };
        this.updatePostStatus = async (postId, userId) => {
            const post = await this.postModel.findOneAndUpdate({
                _id: postId,
                user: userId,
