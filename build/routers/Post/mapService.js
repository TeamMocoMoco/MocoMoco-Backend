...
class MapService {
    constructor() {
        ...
        this.getLocationBySearch = async (keyword) => {
            // 키워드에 제일 근접한 장소 한 곳 검색
            const findPlaceFromText = await axios.get(`https://maps.googleapis.com/maps/api/place/findplacefromtext/json?inputtype=textquery&fields=geometry&input=` +
                encodeURI(keyword) +
            ...
            const response = await axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${coordinate}&radius=5000&language=ko&keyword=` +
                encodeURI(keyword) +
                `&key=${process.env.GOOGLE_API_KEY}`);
            return response;
        };
        // func 장소 추가 검색 시 토큰으로 검색
        this.getLocationByToken = async (token) => {
        ...
        this.getLatLng = async (LatLng) => {
            const indexOfComma = LatLng.indexOf(",");
            const Lat = +LatLng.slice(0, indexOfComma);
            const Lng = +LatLng.substr(indexOfComma + 1);
            return { Lat, Lng };
        };
        // func 지도의 중앙 지점을 받을 시 중앙에서 가까운 순으로 post 검색
        this.getMapPostsByCenter = async (Lat, Lng) => {
        ...
        // 위도경도를 의도적으로 왜곡
        this.randomizeLocation = async (posts) => {
            posts.map((post) => {
                if (post.location) {
                    const randLat = utile_1.rand(-400, 400);
                    const offsetLat = randLat * 0.000001;
                    post.location[0] = Number((post.location[0] + offsetLat).toFixed(6));
                    const randLng = utile_1.rand(-400, 400);
