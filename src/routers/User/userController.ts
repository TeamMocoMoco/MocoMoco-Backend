...
export default class UserController implements Controller {
  ...
  private initializeRoutes() {
    ...
    this.router.get(
      `${this.path}/participant/:userId`,
      this.getParticipantPost
    );
      ...
      const user = await this.userService.updateUser(
        {
          ...
          introduce: "",
          userImg:
            "https://mocomoco.s3.amazonaws.com/original/1620694702756profile_img.png",
        },
        userId
      );
      return res.send({ result: user });
  ...
  private getUserPost: RequestHandler = async (req, res, next) => {
    ...
    try {
      if (status === "true") {
        const activePost = await this.userService.getUserPost(
          page2,
          userId,
          true
        );
        ...
        const deactivePost = await this.userService.getUserPost(
          page2,
          userId,
          false
        );
  ...
  private getParticipantPost: RequestHandler = async (req, res, next) => {
    ...
    try {
      if (status === "true") {
        const activePost = await this.userService.getParticipantsPost(
          page2,
          userId,
          true
        );
        ...
        const deactivePost = await this.userService.getParticipantsPost(
          page2,
          userId,
          false
        );
